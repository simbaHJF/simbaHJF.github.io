...
*	范围查找就往后继续找,找到id=15这一行停下,因此需要加next-key lock(10,15].**注意这是15这个点不会退化,优化2是针对等值查询的**
...
根据原则1和优化2,这个delete和渔具在索引c上的加锁范围,如下图蓝色区域覆盖的部分.蓝色区域左右两边都是虚线,表示开区间,即(c=5,id=5)和(c=15,id=15)这两行上都没有锁.
...
事实上,session B的"加next-key lock(5,10]"操作,实际上分成了两步,显示加(5,10)的间隙锁,加锁成功(前面讲间隙锁定义的时候提到,它不会阻塞其他的gap锁,因此可以加锁成功);然后加c=10的行锁,这时候才被锁住的.
