...
具体点来说,以decode为例,Codec2按照协议规则(默认dubbo)解码消息头(这里包括对消息TCP半包和粘包的处理),将消息体数据流(ChannelBufferInputStream) 封装成 DecodeableRpcResult , 传给给后面的 DecodeHandler ,内部调用Serialization(默认为hessian2)以完成消息体反序列化为对象.<br>
