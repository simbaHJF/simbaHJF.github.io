...
<font color="red">center收到读取配置的请求后,查询本地的缓存文件,将配置内容写入response,而不是直接在内存中缓存配置内容,内存中只缓存md5校验和.这样做的原因是,当配置越来越多时,如果配置内容放在内存中会受单机内存容量瓶颈的限制,而放在磁盘则几乎不会有问题,因为磁盘容量可以做到很大;对于md5校验和,它占空间很小,全放内存中没问题.对于读写能力的问题,则可以通过横向扩展集群规模的方式来解决</font>
...
这里,配置变更时,center节点对center集群中其他进行通知时,如果http请求失败了,会加入延迟异步通知任务中,后面再次进行通知.这是一种最大努力尝试方案.最终会达到一致性.<br>
